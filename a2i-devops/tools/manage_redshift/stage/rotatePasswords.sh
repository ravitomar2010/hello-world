...
updatePasswords(){
  ...
  while read line; do
  	if [[ $line == "" ]]; then #if1
        ...
        temp_entry_count=`aws ssm get-parameter --name "$line" --with-decryption --profile stage --output text --query Parameter.Value | wc -l`
...
updatePasswordsForSingleEntry(){
  ...
  aws ssm put-parameter --name "/a2i/stage/redshift/users/$local_uname" --value "$new_pass" --type SecureString --overwrite --profile stage
...
updatePasswordsForDoubleEntry(){
  ...
  echo "new_pass is $new_pass"
  aws ssm put-parameter --name "/a2i/stage/redshift/users/$local_uname" --value "$new_pass" --type SecureString --overwrite --profile stage
  aws ssm put-parameter --name "$local_uname" --value "$new_pass" --type SecureString --overwrite --profile stage
