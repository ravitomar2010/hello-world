...
checkIfUserExistsAndMakeEntry(){
  ...
  if [[ $local_schema_name == *"dbo"* ]]; then
    ...
    userentryflag=`cat ../../manage_redshift/stage/users_list.txt  | grep "batch_$temp_schema_name" | wc -l`
    groupentryflag=`cat ../../manage_redshift/stage/groups_list.txt  | grep "mstr_batch_$temp_schema_name" | wc -l`
    ...
    if [[ $userentryflag -eq 0 ]]; then
        ...
        echo "batch_$temp_schema_name mstr_batch_$temp_schema_name" >> ../../manage_redshift/stage/users_list.txt
    fi
    if [[ $groupentryflag -eq 0 ]]; then
        echo "Group entry doesnt exists"
        echo "mstr_batch_$temp_schema_name" >> ../../manage_redshift/stage/groups_list.txt
    fi
  ...
  if [[ $line == "" ]]; then #if 1
    echo "Skipping empty line"
      ...
      if [[ $isschemaexists -gt 0 ]]; then #if 2
          ...
          #cat ../../manage_redshift/stage/users_list.txt
      else
