...
module "vpc" {
  ...
  infra_subnets     = [cidrsubnet(local.variables[terraform.workspace].vpc_cidr, 7, 5), cidrsubnet(local.variables[terraform.workspace].vpc_cidr, 7, 6)]
  ...
  infra_route_table_tags = {
    Name        = "${var.platform}-infra-rt"
  }
...
resource "aws_route" "route_for_cf_for_infra" {
...
resource "aws_route" "route_for_dc_for_infra" {
  route_table_id            = module.vpc.infra_route_table_ids[0]
  destination_cidr_block    = "172.27.0.0/16"
  transit_gateway_id        = local.variables[terraform.workspace].transit_gateway_id
  depends_on                = [module.vpc]
...
resource "aws_route" "route_for_hyke-prod_for_infra" {
  route_table_id            = module.vpc.infra_route_table_ids[0]
  destination_cidr_block    = "10.80.0.0/16"
  transit_gateway_id        = local.variables[terraform.workspace].transit_gateway_id
  depends_on                = [module.vpc]
...
resource "aws_route" "route_for_hyke-stage_for_infra" {
  route_table_id            = module.vpc.infra_route_table_ids[0]
  destination_cidr_block    = "10.81.0.0/16"
  transit_gateway_id        = local.variables[terraform.workspace].transit_gateway_id
  depends_on                = [module.vpc]
...
resource "aws_route" "route_for_hyke-dev_for_infra" {
  route_table_id            = module.vpc.infra_route_table_ids[0]
  destination_cidr_block    = "10.50.0.0/16"
  transit_gateway_id        = local.variables[terraform.workspace].transit_gateway_id
  depends_on                = [module.vpc]
...
resource "aws_route" "route_for_hyke-qa_for_infra" {
  route_table_id            = module.vpc.infra_route_table_ids[0]
  destination_cidr_block    = "10.60.0.0/16"
  transit_gateway_id        = local.variables[terraform.workspace].transit_gateway_id
  depends_on                = [module.vpc]
...
resource "aws_route" "route_for_hyke-APIs_for_infra" {
  route_table_id            = module.vpc.infra_route_table_ids[0]
  destination_cidr_block    = "10.70.0.0/16"
  transit_gateway_id        = local.variables[terraform.workspace].transit_gateway_id
  depends_on                = [module.vpc]
