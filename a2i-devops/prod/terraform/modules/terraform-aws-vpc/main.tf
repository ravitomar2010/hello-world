...
resource "aws_subnet" "infra" {
  ...
  ipv6_cidr_block = var.enable_ipv6 && length(var.private_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.infra_subnet_ipv6_prefixes[count.index]) : null
...
################################################################################
#Network ACLs for each subnet_ids
################################################################################
...
resource "aws_network_acl_rule" "infra_inbound" {
  ...
  ipv6_cidr_block = lookup(var.infra_inbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_network_acl_rule" "infra_outbound" {
  ...
  ipv6_cidr_block = lookup(var.infra_outbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
