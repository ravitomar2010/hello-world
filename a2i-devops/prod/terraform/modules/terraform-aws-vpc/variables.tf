...
variable "infra_subnet_ipv6_prefixes" {
  description = "Assigns IPv6 infra subnet id based on the Amazon provided /56 prefix base 10 integer (0-256). Must be of equal length to the corresponding IPv4 subnet list"
  type        = list
  default     = []
}
...
variable "infra_subnet_assign_ipv6_address_on_creation" {
  description = "Assign IPv6 address on infra subnet, must be disabled to change IPv6 CIDRs. This is the IPv6 equivalent of map_public_ip_on_launch"
  type        = bool
  default     = null
}
...
variable "infra_subnet_suffix" {
  description = "Suffix to append to infra subnets name"
  type        = string
  default     = "infra"
}
...
variable "infra_route_table_tags" {
  description = "Additional tags for the infra route tables"
  type        = map(string)
  default     = {}
}
...
variable "infra_acl_tags" {
  description = "Additional tags for the infra subnets network ACL"
  type        = map(string)
  default     = {}
}
...
variable "infra_dedicated_network_acl" {
  description = "Whether to use dedicated network ACL (not default) and custom rules for infra subnets"
  type        = bool
  default     = false
}
...
variable "infra_inbound_acl_rules" {
  description = "Infra subnets inbound network ACLs"
  type        = list(map(string))
...
variable "infra_outbound_acl_rules" {
  description = "Infra subnets outbound network ACLs"
  type        = list(map(string))
