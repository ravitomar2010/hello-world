...
checkIfPolicyExistsAndCreate(){
    ...
    echo "I am checking on policy"
    #policy_details=`aws iam list-policies --scope Local --profile $profile`
    ...
    echo "policy details are $policy_details"
    echo "policy exists count $is_policy_exists"

    if [[ $is_policy_exists -gt 1 ]]; then
