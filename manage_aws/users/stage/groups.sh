			...
			isgroupexists=`echo $existinggrouplist | grep $group_name| wc -l`
					...
					if [[ $isgroupexists -gt 0 ]]; then
						#statements
						 ...
						 # group_name=$line
						  aws iam create-group --group-name $group_name --profile stage
							aws iam attach-group-policy --group-name $group_name --policy-arn arn:aws:iam::aws:policy/IAMUserChangePassword --profile stage
					fi
	fi
done < $filename
