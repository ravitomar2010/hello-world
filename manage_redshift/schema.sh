  ...
  while read line; do
    ...
    if [[ $line == "" ]]; then #if 1
      echo "Skipping empty line"
        ...
        schemaName=$line
        isSchemaExists=`echo $existingSchemaList | grep -i -w $schemaName | wc -l`
...
checkAndResolveDependency(){
  ...
  if [[ $localSchemaName == *"dbo"* ]]; then
        ...
        groupEntryFlag=`cat tmpGroupsList.txt | grep -i -w "mstr_batch_$tempSchemaName" | wc -l`
        userEntryFlag=`cat tmpUsersList.txt  | grep -i -w "batch_$tempSchemaName" | wc -l`
        accessEntryFlag=`cat tmpAccessList.txt | grep -i -w "mstr_batch_$tempSchemaName" | wc -l`
