...
checkIfTableExists(){
  ...
  if [ $isTableExists -eq 0 ];then
      ...
      tableDefQuery1=`echo "$tableDefQuery" | sed "s|${sourceSchemaName}|${schemaName}|g"`
      echo query is $tableDefQuery
      executeQueryAndGetResults "${tableDefQuery1}"
  else
...
checkTableStructure(){
  ...
  echo "Number of Columns in prod is $countProdColumn"

  getTableStructureStage

  echo "Number of Columns in stage is $countStageColumn"
...
getTabledef(){
     ...
     tableDefQuery=`echo "$results"| sed 's/^.*CREATE/CREATE/'`
     #tableDefQuery=`echo "$results"| sed -i '1d'`
}
...
CheckTableDataSizeOrFullDump(){
  ...
  findDateColumn
    ...
    if [[ $tablesizeresult -le 500 ]] || [[ $fullDump == "YES" ]]; then
        ...
        executeUnloadQuery
        echo "<div style="color:Navy">Table: <b>$tableName</b>  TransferedDataSize: <b>$tablesizeresult MB</b></div><br>" >> listoftableswithsize.txt
...
prepareOutput(){
  ...
  echo "<div style="color:Navy">Hi All <br><br>$BUILD_USER_EMAIL migrated below list of tables of $schemaName schema to STAGE environment</div>" >> tmpFinalOP.txt
  echo "<div style="color:Navy"><br>=============================================================</div>" >> tmpFinalOP.txt
  cat listoftableswithsize.txt >> tmpFinalOP.txt
  echo "<div style="color:Navy">==============================================================</div>" >> tmpFinalOP.txt

  echo "<div style="color:Navy"><br>Please reach out to devops in case of any concern<br><br>Regards<br>DevOps Team</div>" >> tmpFinalOP.txt
  echo '</pre>' >> tmpFinalOP.txt
...
getSSMParameters(){
  ...
  leadsMailLists=`aws ssm get-parameter --name /a2i/${profile}/ses/leadsMailList --profile ${profile} --with-decryption --query Parameter.Value --output text`
...
sendMailfulldump(){
      ...
      aws ses send-email \
      --from "$fromEmail" \
      --destination "ToAddresses=$toMail","CcAddresses=$leadsMailLists" \
