...
checkIfUserExistsAndMakeEntry(){
  ...
  if [[ $local_schema_name == *"dbo"* ]]; then
    ...
    groupentryflag=`cat groups_list.txt  | grep "mstr_batch_$temp_schema_name" | wc -l`
    ...
    if [[ $userentryflag -eq 0 ]]; then
        ...
        echo "batch_$temp_schema_name mstr_batch_$temp_schema_name" >> users_list.txt
    fi
    if [[ $groupentryflag -eq 0 ]]; then
        echo "Group entry doesnt exists"
        echo "mstr_batch_$temp_schema_name" >> groups_list.txt
    fi
    ...
    if [[ $accessentryflag -eq 0 ]]; then
        echo "Access entry doesnt exists"
        echo "mstr_batch_$temp_schema_name MASTER $local_schema_name, ${temp_schema_name}_stage" >> access_list.txt
        #echo "mstr_batch_$temp_schema_name MASTER {$temp_schema_name}_dbo, " >> groups_list.txt
    fi
  else
      ...
      if [[ $accessentryflag -eq 0 ]]; then
          echo "Access entry doesnt exists"
          echo "mstr_batch_$temp_schema_name MASTER $local_schema_name" >> access_list.txt
          #echo "mstr_batch_$temp_schema_name MASTER {$temp_schema_name}_dbo, " >> groups_list.txt
      fi
  fi
