...
def grantAccess(groupName, permission, profile):
    base_path = str(Path(os.getcwd()))
    print('Executing access.sh')
    if dbClient == 'axiom':
        command_to_run=str('/bin/bash '+base_path+'/../access.sh modifyIndividual@'+dbClient+' '+groupName+'@'+permission+' '+profile)
        print(command_to_run)
        os.system(command_to_run)
    elif dbClient == 'hyke':
        command_to_run=str('/bin/bash '+base_path+'/../access.sh modifyIndividual@'+dbClient+' '+groupName+'@'+permission+' '+profile)
...
grantAccess(groupName, permission, profile)

# subject=profile+" | Redshift access request status "
#
# user=email.split('.')[0].capitalize()
#
# l_data_to_send='Hi '+user+' <br> <br> You have been assigned with the <b>'+permission+'</b> permission for <b>'+schemaName+'</b> schema from <b>'+dbClient+'</b> database in <b>'+profile+'</b> environment'
# sendNotification(l_data_to_send,profile)
# time.sleep(5)
# l_data_to_send='Hi All, <br> <br> This is to notify you that <b>'+user+'</b> have granted himself <b>'+permission+'</b> permission for <b>'+schemaName+'</b> schema from <b>'+dbClient+'</b> database in <b>'+profile+'</b> environment <br><br> Regards <br> DevOps Team'
