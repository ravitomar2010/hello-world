...
def executeQueryAndGetViewList(hostName,portNo,dbName,redshiftPassword,accountID,redshiftUserName,sql):
    print('Executing query ')
    conn = psycopg2.connect(dbname=dbName, user=redshiftUserName, password=redshiftPassword, port=portNo, host=hostName)
    cur = conn.cursor();
    cur.execute(sql)
    res = cur.fetchall()
    #res=queryResultParser(res)
    header_list = ["schemaname", "viewname"]
    data = pd.DataFrame(res, columns=header_list)
    return data
...
def createViewTextFile(result,filename):
...
print('Fetching view list from database')
viewNameSQL="select schemaname, viewname from pg_views where schemaname not in ('admin','pg_catalog','public','information_schema') order by viewname"

result=executeQueryAndGetViewList(hostName,portNo,dbName,redshiftPassword,accountID,redshiftUserName,viewNameSQL)

#result.style.set_properties(**{'text-align': 'right'})
print('creating view list files')

for schema in result.schemaname.unique():
    viewListToSave = (result[result.schemaname==schema].viewname.to_string(index=False))
    viewListToSave=(viewListToSave.replace(' ',''))
    tmpFilename =baseDir+'/'+profile+'/'+dbClient+'/'+schema+'/viewList.txt'
    createViewTextFile(viewListToSave,tmpFilename)
#######################################################################
############################# Clean - up  #############################
#######################################################################
