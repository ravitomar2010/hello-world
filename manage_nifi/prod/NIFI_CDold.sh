...
GetBucketID(){
        ...
        bucket_id=$(curl --silent ${url} | jq '.[] | select(.name == '\"$bucket_name\"') | .identifier' | sed 's/"//g')
        if [ -z "$bucket_id" ]; then
...
GetFlowId(){
        flow_id=$(echo "$bucket_flows" | jq -r '.[] | select(.name == '\"$flow_name\"' ) | .identifier' | sed 's/"//g' )
        if [ -z "$flow_id" ]; then
...
GetRegistryId(){
        ...
        echo $url
        registry_id=$(curl --silent -H "Authorization: Bearer $token" ${url} --insecure | jq ".registries[0].id" | sed 's/"//g')
...
GetLatestFlowVersion(){
        latest_flow_version=$(echo "$bucket_flows" | jq -r '.[] | select(.name == '\"$flow_name\"' ) | .versionCount' | sed 's/"//g' )
...
GetNifiFlowId_Version(){
        nifi_flow_id=$( echo "$nifi_pg_list" | jq '.processGroups[].component.versionControlInformation | select(.flowId == '\"$flow_id\"') | .groupId' | sed 's/"//g')
        nifi_current_version=$( echo "$nifi_pg_list" | jq '.processGroups[].component.versionControlInformation | select(.flowId == '\"$flow_id\"') | .version')
        if [ ! -z "$nifi_flow_id" ]
        then
...
new_pg_flow_id=$(echo "$import_flow" | jq '.id' | sed 's/"//g')

if [ ! -z "$new_pg_flow_id" ]
then
        echo "Flow <b>$flow_name</b> created on <b>Nifi Stage</b> with id $new_pg_flow_id<br>" >> flowstatus.txt
...
EOF
        ...
        request_id=$(echo "$update_flow_request" | jq '.request.requestId' | sed 's/"//g')
        ...
        sleep 5
        echo "Nifi Flow $flow_name updated with latest version $latest_flow_version on stage environment<br>" >> flowstatus.txt
...
sendStatusMail(){
  ...
  --message "Subject={Data=STAGE | CI-CD | NIFI Flow $flow_name Migration Notification,Charset=utf8},Body={Text={Data=Testing Body,Charset=utf8},Html={Data=<p style="color:blue"> Hi Team <br><br>This is to notify that new version of NIFI Flow entitled as <b>$sourceFlowName</b> is attempted to migrate on <b>STAGE</b> Environment.Please find below details:</p><p style="color:black">$(cat flowstatus.txt)</p><p style="color:blue">Please reach out to DevOps team in case of any issue or concerns.<br><br>Thanks and Regards <br> DevOps Team.</p> ,Charset=utf8}}" \
  --profile $profile
