...
/*
  ...
  */  
    ...
    when {  
      ...
      steps{
//        script {
//          deployEKS.installerpGradleIstioChart()
//        }
        script {
          deployEKS.installProdErpChart()
        }        
        echo "App should be accessible on url: http://axiom-erp-dev-pvt.hykeapi.com/${env.APP_NAME}/actuator/health"
      }
    }
    
    stage('Build Release (Production)') {
      when { allOf { branch 'master'; not { changeset "Makefile" } } }
      environment {
        PREVIEW_VERSION = "$BRANCH_NAME-$BUILD_NUMBER"
        PREVIEW_NAMESPACE = "$APP_NAME-$BRANCH_NAME".toLowerCase()
        HELM_RELEASE = "$PREVIEW_NAMESPACE".toLowerCase()
      ...
      steps {
        ...
        // ensure we're not on a detached head
          ...
          sh """
             ...
             git add . && git commit -a -m \"release \$(cat VERSION)\" || :
