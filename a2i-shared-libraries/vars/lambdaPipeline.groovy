...
def call() {
	pipeline {
		...
		environment {
			...
			LAYER_PROD = 'arn:aws:lambda:eu-west-1:530328198985:layer:a2i-lambda-layer:24'
			LAYER_STAGE = 'arn:aws:lambda:eu-west-1:403475184785:layer:a2i-lambda-layer:17'
			MEMORY = '512'
			ROLE_PROD = 'arn:aws:iam::530328198985:role/a2i-lambda-role'
			ROLE_STAGE = 'arn:aws:iam::403475184785:role/a2i-lambda-role'
			TIMEOUT = '900'
		}
		stages {

				stage('Create code archives') {
					when { anyOf { branch 'developer' ; branch 'qa' ; branch 'master' ; branch 'dev_prakhar'} }
					steps{
								...
								if [ -d "\$dir" ]; then
										...
										case "\$LAMBDA_INFO" in
										  ...
										  *       )
										    ...
										    --vpc-config SubnetIds=subnet-00c0846e72d7d41c6,subnet-053ec79e10f124fee,SecurityGroupIds=sg-00bc8d46f601fdd69 \
										    --region eu-west-1 \
										    --environment Variables="{environment=\${BRANCH_NAME}}" \
										    --layers \$LAYER_STAGE \
										    --profile stage
				 				...
				 				do
				 					...
				 					########## Actual deployment code
				 						...
				 						*       )
				 							...
				 							--zip-file fileb://${ORG}-${APP_NAME}-\${dir}.zip \
				 							--publish --memory-size ${MEMORY} \
											--vpc-config SubnetIds=subnet-0f0ed0f905c777b38,subnet-0085533bac9ddf3a6,SecurityGroupIds=sg-0579fce65402c2f71 \
				 							--region eu-west-1 \
				 							--layers \$LAYER_PROD \
				 							--environment Variables={environment=production} \
											--tracing-config Mode=Active \
				...
				stage('SonarQube Analysis for dev_prakhar branch') {
						environment {
								 SONARRUNER_HOME = tool name: 'sonarRunner', type: 'hudson.plugins.sonar.SonarRunnerInstallation'
						 }

								 when { anyOf { branch 'dev_prakhar' } }
								 steps {
									...
									aws ses send-email \
									--from "a2iteam@axiomtelecom.com" \
									--destination "ToAddresses=prakhar.gandhi@intsof.com","CcAddresses=yogesh.patil@axiomtelecom.com,axiomdipoffshoredev@intsof.com" \
									--message "Subject={Data=SonarQube Analysis ${JOB_NAME} Report,Charset=utf8},Body={Text={Data=Testing Body,Charset=utf8},Html={Data=Hi Team <br><h1 style="color:blue">SonarQube Analysis Report</h1><br><b>ProjectName:</b> ${JOB_NAME}<br><b>Branch:</b> ${BRANCH_NAME}<br><b>Build Number:</b> ${BUILD_ID}<br><br><table BORDER=10 BORDERCOLOR="#0000FF" BORDERCOLORLIGHT="#33CCFF" BORDERCOLORDARK="#0000CC" width= 40% >
								 <tr padding= 8px>
								 <th colspan=2 border= 10px solid #dddddd padding= 8px style="text-align:center"><font face="Arial" color="BLACK">CODE QUALITY REPORT</font></th>
								 </tr>
