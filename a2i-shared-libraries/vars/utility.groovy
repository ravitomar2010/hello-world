...
def getECRName(){
    String repo = steps.sh(returnStdout: true, script: "aws ecr get-login --region eu-west-1 --no-include-email | cut -d \" \" -f 7" ).trim()
...
def createDockerRegistry() {
    def sout = new StringBuilder(), serr = new StringBuilder()
    command = "aws ecr describe-repositories --region eu-west-1"
    echo command
    def proc = command.execute()
    proc.consumeProcessOutput(sout, serr)
    proc.waitForOrKill(1000)
...
def createDockerRegistry(def dockerRepo) {
    ...
    if(reg_found == true){
        ...
        command = "aws ecr create-repository --repository-name ${dockerRepo} --region eu-west-1"
...
def createDockerRegistrydata(def dockerRepo) {
    def sout = new StringBuilder(), serr = new StringBuilder()
...
def createDockerfile(){
    if (fileExists('build.gradle')){
        ...
        def dockerfile = libraryResource 'Dockerfile.node.gradle'
...
def createKubeConfigProd(){
    if (fileExists('/var/lib/jenkins/.kube/config_prod')){
        println('Kubeconfig for production found not creating...')
    }
    else{
        println('Creating kubeconfig for production...')
        def kubeConfig = libraryResource 'config_prod'
        writeFile file: '/var/lib/jenkins/.kube/config_prod', text: kubeConfig
    }
}        

def createKubeConfigErpProd(){
    if (fileExists('/var/lib/jenkins/.kube/config_erp_prod')){
        println('Kubeconfig for Erp found not creating...')
    }
    else{
        println('Creating kubeconfig for Erp...')
        def kubeConfig = libraryResource 'config_erp_prod'
        writeFile file: '/var/lib/jenkins/.kube/config_erp_prod', text: kubeConfig
    }
} 

def createTD(){
    def td = libraryResource 'ecs_td.template'
    println('Creating template for gradle project...')
