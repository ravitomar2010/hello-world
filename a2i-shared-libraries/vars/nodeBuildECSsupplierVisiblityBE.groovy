...
*/
      stage('Docker Build & Artefact') {
        ...
        steps {
          ...
          //sh "sed -i -e \"s/RUN npm install//\" Dockerfile"
          sh "sed -i -e \"s/RUN npm run PHASE//\" Dockerfile"
          sh "sed -i -e \"s/3001/3003/\" Dockerfile"
      ...
      stage('Deploy to ECS non-prod') {
        ...
        steps {
          script{
            utility.createTD()
            def repo_result = utilityData.createDockerRegistry("${ORG}/${APP_NAME}-${ENV_NAME}")
            //if (initECS.isService()){
              println('Updating service...')
              def FE_IMAGE = utilityData.getECRRepoLatestTag("axiom-telecom/a2i-data-supplier-visibility-${ENV_NAME}")
              initECS.initTaskDefinition_BE(FE_IMAGE)
              
            //}
            //else { */
              println('Creating cluster and service...')
              //initECS.createCluster()
              //initECS.initTaskDefinition_BE()
              def ver = initECS.getVersionSupplier()
              initECS.updateService(ver)
              //def lbARN = initECS.createLB()
              //def tgARN = initECS.createTargetGroup()
              //println(lbARN)
              //println(tgARN)
              //initECS.createListener(tgARN, lbARN)
              //initECS.createService(tgARN)
            //}
          }
        }
      }
