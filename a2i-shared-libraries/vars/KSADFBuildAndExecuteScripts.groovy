				...
				when { anyOf { branch 'master'} }
				  steps{
				    sh """
							...
							/data/venvs/prod-df-ksa/bin/python3 ./scripts/aws_dataset_input.py $env.GIT_BRANCH
				...
				when { anyOf { branch 'master'} }
				  steps{
				    sh """
				      ...
				      /data/venvs/prod-df-ksa/bin/python3 ./scripts/safety-stock.py $env.GIT_BRANCH
				...
				when { anyOf { branch 'master'} }
					steps{
						sh """
							...
							#sudo rm -rf /var/lib/jenkins/workspace/production-df-init_master/prod-df-ksa
				...
				when { anyOf { branch 'developer' ; branch 'qa' } }
				  steps{
				    sh """
				       ...
				       /data/venvs/stage-df-ksa/bin/python3 ./scripts/aws_dataset_input.py $env.GIT_BRANCH
				...
				when { anyOf { branch 'developer' ; branch 'qa' } }
				  steps{
				    sh """
				      ...
				      /data/venvs/stage-df-ksa/bin/python3 ./scripts/safety-stock.py $env.GIT_BRANCH
			...
			stage('Perform cleanup for stage') {
				when { not { branch 'master'} }
					steps{
						sh """
							echo "For now I am not cleaning virtual environment"
							#sudo rm -rf /var/lib/jenkins/workspace/stage-df-ksa-init_developer/stage-df-ksa
