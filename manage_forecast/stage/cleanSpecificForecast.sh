...
profile='stage'
datasetGroup='UDI04159_Jul_02_2020_26_29_ds_grp,UDI01625_Jul_02_2020_26_19_ds_grp'
#######################################################################
############################# Generic Code ############################
#######################################################################
...
getDatasetGroupArn(){
  queryToReplace=DatasetGroups[?DatasetGroupName==\`${dsg}\`][DatasetGroupArn][0]
  dsgArn=$(aws forecast list-dataset-groups --profile stage --output text --query $queryToReplace)
  echo "I have received dsgARN as $dsgArn"

  findAndDeleteForecastExports
...
for dsg in $(echo $datasetGroup | sed "s/,/ /g")
do
	echo "Working on $dsg"
  getDatasetGroupArn
done
